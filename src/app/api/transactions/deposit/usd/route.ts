...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingDeposit = await prisma.usdDeposit.findFirst({
      where: { txHash: txHash },
    });
    ...
    const blockchainService = getUsdtBlockchainService();
    ...
    // Convert to blockchain units (6 decimals for USDT)
    const depositAmountBigInt = BigInt(Math.floor(parseFloat(actualAmount) * 1000000));
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const deposit = await tx.usdDeposit.create({
        data: {
          userId: userId,
      ...
      await tx.usdTransaction.create({
        data: {
          ...
          description: `Deposit confirmed: ${actualAmount} USD from transaction ${txHash}`,
      ...
      return {
        deposit,
        newBalance: updatedUser.usdBalance.toString(),
      };
    });
    ...
    return NextResponse.json({
      ...
      message: `Deposit of ${actualAmount} USD confirmed and added to your account`,
    });

  } catch (error) {
    console.error('Error creating USD deposit:', error);
    return NextResponse.json(
