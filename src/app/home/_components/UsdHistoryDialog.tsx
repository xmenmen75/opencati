...
import { History, TrendingUp, Package, Trophy, ArrowLeft } from 'lucide-react';
import { useUsdTransactions } from '@/hooks/queries';
import { Loading } from '@/components/ui/Loading';
import { formatCurrency, CurrencyType } from '@/lib/currency';

interface UsdHistoryDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export function UsdHistoryDialog({ isOpen, onClose }: UsdHistoryDialogProps) {
  const [currentPage, setCurrentPage] = useState(0);
  const pageSize = 10;
  
  const { data, isLoading, error, refetch } = useUsdTransactions(pageSize, currentPage * pageSize);
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-900">
                      {data.summary.totalPackOpenings}
                    </div>
                    <div className="text-xs text-gray-500">Pack Opens</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-red-600">
                      {formatCurrency(data.summary.amountSpent / 1e6, CurrencyType.USD)}
                    </div>
                    <div className="text-xs text-gray-500">USD Spent</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-green-600">
                      {formatCurrency(data.summary.amountDeposited / 1e6, CurrencyType.USD)}
                    </div>
                    <div className="text-xs text-gray-500">USD Deposited</div>
                  </div>
                  <div className="text-center">
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {data.transactions.map((transaction: any) => {
                          ...
                          const spentAmount = amount < 0 ? Math.abs(amount) / 1e6 : 0;
                          const depositedAmount = amount > 0 ? amount / 1e6 : 0;
                          ...
                          return (
                            <tr key={transaction.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-right text-red-600">
                                {spentAmount > 0 ? formatCurrency(spentAmount, CurrencyType.USD) : '-'}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-right text-green-600">
                                {depositedAmount > 0 ? formatCurrency(depositedAmount, CurrencyType.USD) : '-'}
                              </td>
                ...
                {data?.pagination && (
                  <div className="flex items-center justify-between px-4 py-3 bg-white border border-gray-200 rounded-lg">
                    <div className="text-sm text-gray-500">
                      Showing {currentPage * pageSize + 1} to {Math.min((currentPage + 1) * pageSize, data.pagination.total)} of {data.pagination.total} transactions
                    </div>
